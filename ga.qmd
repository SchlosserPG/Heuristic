...
-   Larger tournaments (e.g., num_players \>= 3) favor stronger individuals.
    -   Increases selection pressure because there's a higher chance that fitter individuals will be included and selected.
    -   This can lead to faster convergence but may reduce genetic diversity, risking premature convergence to suboptimal solutions.
-   Smaller tournaments (e.g., num_players = 2) introduce more randomness. + Decreases selection pressure, allowing less fit individuals a better chance to be selected. This maintains diversity and promotes exploration but may slow down convergence.
...
-   F1 (Parent 1) = 11001 \| 10011

-   F3 (Parent 2) = 10101 \| 01010

-   Let's choose crossover point or index = 5 , which means after the fifth bit

    -   Child 1 = 11001 \| 01010 → "1100101010"
    -   Child 2 = 10101 \| 10011 → "1010110011"
...
-   Because the Ackley problem uses **real-valued** solutions (continuous numbers between -10 and 10) rather than binary bit strings, we use **arithmetic (blend) crossover** instead of the single-point crossover used in the One-Max problem.

-   Arithmetic crossover works by blending the two parent values using a random mixing factor $\alpha \in [0, 1]$:
...
-   $\alpha$ is drawn uniformly from $[0, 1]$ for each pair, so offspring always lie between the two parents. This is appropriate for continuous solution spaces because it smoothly blends parental values rather than swapping discrete chunks.

-   The loop iterates over the population in pairs. For each pair, if a random number is less than the `crossover_rate`, crossover occurs using the blending formula. Otherwise, the parents pass through unchanged. The function returns the new population after crossover.

```{python}
# Transition function (T): Crossover and Mutation
# Uses arithmetic (blend) crossover for real-valued solutions. It creates two offspring
# by blending the parent solutions based on a random factor alpha.
def crossover(population, crossover_rate):
    new_population = population.copy()
    for i in range(0, len(population) - 1, 2):
        if random.random() < crossover_rate:
...
-   Because we are working in a **continuous** solution space, we use **Gaussian mutation** rather than a random replacement. Each individual is mutated with a probability specified by `mutation_rate`. When mutation occurs, a small random value drawn from a normal distribution $\mathcal{N}(0, \sigma)$ is **added** to the current value:

    $$x_{\text{new}} = x_{\text{old}} + \mathcal{N}(0, \sigma)$$

-   The parameter $\sigma$ (sigma) controls the **step size** — how far from the current value the mutation can move. A small $\sigma$ (e.g., 0.5) produces small perturbations, allowing fine-grained local exploration. A large $\sigma$ produces bigger jumps, allowing more exploration of the broader solution space. This is fundamentally different from the uniform random replacement used in some earlier versions, which could produce large, disruptive jumps regardless of where the current solution is. Gaussian mutation is gentler and more directed, nudging solutions in small steps rather than teleporting them.

-   In the code below, `sigma=0.5` means mutations will typically shift the value by about ±0.5 units.

```{python}
# Mutation function (T)
# Applies Gaussian mutation to the population. Each individual has a chance
# to be mutated based on the mutation_rate. If selected for mutation, a small
# random value drawn from a normal distribution N(0, sigma) is added to it,
# allowing for controlled exploration of the solution space.
# sigma is the standard deviation of the normal distribution, which controls
# the extent of the mutation step size.
def mutation(pop, mutation_rate, sigma=0.5):
...
def genetic_algorithm():
    ...
    for generation in range(num_generations):
        ...
        offspring_pop = crossover(selected_pop, crossover_rate)
        mutated_pop = mutation(offspring_pop, mutation_rate, sigma=0.5)
