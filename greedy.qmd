...
# Nearest Neighbor using Dataset

```{python}
import pandas as pd

# Load with first column as index
# Always do a check on the data file to ensure it's clean and ready to use. 
# Check how the data loaded. 
df = pd.read_csv("data/twentylocations.csv", index_col=0)

# (Optional) clean labels
df.index = df.index.str.strip()
df.columns = df.columns.str.strip()

# Turn into dict-of-dicts keyed by row (L1, L2, …)
dist = df.to_dict(orient="index")

print(df.shape)          # should be (20, 20)
print(df.head())         # should show L1…L5 rows and columns

start_location = "L1"

def nearest_neighbor(start, dist):
    current = start
    visited = {start} ##visited is a set because we only care about membership, not order. Asks "Have I been here?"  
    route = [start] ## route is a list because we do care about order. Asks: What order did I visit? 
    total_distance = 0
    ...
    while len(visited) < len(dist):
        ...
        # Look at all neighbors of the current location
        for neighbor, d in dist[current].items():
            if neighbor in visited:
                continue  # skip locations we've already visited

            # Choose the unvisited neighbor with the smallest distance
...
route, total = nearest_neighbor(start_location, dist)

print("Nearest Neighbor route starting at", start_location, ":")
print(" -> ".join(route))
print("Total distance:", total)
```
