...
-   In the field of heuristic algorithms, coding know-how is not just a tool—it’s an essential skill that enables you to experiment, adapt, and optimize solutions for complex, real-world problems. As we explore heuristic methods in this MSBA course, having a strong foundation in coding, especially in Python, will allow you to implement and test algorithms efficiently.
...
-   The Ackley function is commonly used as a benchmark problem in optimization, and is known for its many local minima. The Ackley function uses the np.sqrt, np.sum, and np.exp within its formula.
...
* Heuristic algorithms use randomness to explore and diversify potential solutions efficiently. These random functions generate diverse candidate solutions or perturbations that guide the search process.
...
* Examples of Random Number Functions
     + np.random.standard_normal() draws values from a normal distribution (mean = 0, sd = 1) to introduce controlled variation.
     + np.random.rand() samples from a uniform [0, 1) distribution for balanced, unbiased exploration.
     + np.random.uniform(a, b) samples uniformly over a specified range, allowing bounded randomization of parameters or positions.
     + np.random.randint(low, high) produces random integer values within a range, often used to pick indices or discrete options in a population.
...
## np Functions from Simulated Annealing example
...
* If only one number is given, np.random.randint generates values from 0 up to (but not including) that number. For example, the code below shows values from generated from 0 to 20 (exclusive) in a 3 row by 5 column view. 
...
## np.random.randint from Simulated Annealing (from One Max)
...
## An Algorithm Modeling Stock Prices
...
data <- data.frame(
  Pattern = c(
    "Single underscore: \"ignore\"",
    "Single underscore: \"internal\"",
    "Double underscore: \"name mangling in a class\"",
    "Double before & after: \"special built-in behavior\""
  ),
  Example = c(
    "for _ in range(10):",
    "_helper_function()",
  ...
  `Common Use in Python` = c(
    "Used as a throwaway variable when the value isn’t needed (e.g., loop counter you don’t use). In some interactive REPLs, `_` can store the last result.",
    "Conventional signal that a function/variable is intended for internal use within a module (and is skipped by `from module import *`).",
    "Triggers name mangling inside a class so attributes are harder to access accidentally from outside (e.g., `__secret` becomes `_MyClass__secret`).",
    "Used for Python \"dunder\" names (double underscore). `__name__ == \"__main__\"` runs code only when the file is executed directly, not imported."
  ),
  `Code Snippet` = c(
    ">>> 10 * 2\n20\n>>> _\n20",
    "def _helper_function():\n    return \"internal use\"\n\ndef public_function():\n    return _helper_function()",
    "class MyClass:\n    def __init__(self):\n        self.__secret = 42\n\nobj = MyClass()\n# obj.__secret  # AttributeError\nobj._MyClass__secret",
...
kable(
  data,
  align = c("l", "l", "l", "l"),
  caption = "Common Uses of the Underscore in Python"
)
