...
-   The Time examples below compare the time it takes to sort lists of 1,000,000 elements using both Python’s built-in sorting method and NumPy’s sorting methods (quicksort, mergesort, heapsort, and timsort).
-   Library Imports:
...
* In NumPy, mergesort is one of the sorting algorithms that can be specified in the np.sort() function. 
* The mergesort algorithm is a stable, comparison-based, divide-and-conquer sorting algorithm that divides the array into halves, recursively sorts each half, and then merges the sorted halves back together.
