...
data <- data.frame(
  Pattern = c(
    ...
    "Double underscore: “private inside a class.”",
  ...
  `Common Use in Python` = c(
    "Used as a throwaway variable when the value isn’t needed. In the REPL, `_` also stores the last result.",
    ...
    "Common structure to run code only when the file is executed directly, not when it’s imported as a module."
  )
)

kable(data, align = c('l', 'l', 'l'), caption = "Common Uses of the Underscore in Python")
```
