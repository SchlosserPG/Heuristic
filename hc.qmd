...
![hc search](Pictures/hc10.png "Simulation Results") \## Comparison of ES and HC for the one-max Problem of size $n=10$
...
* ARIMA is used as a forecasting pre-processing step to estimate future expected returns for each asset. Hill climbing then treats those forecasts as fixed inputs and optimizes portfolio weights to maximize the Sharpe ratio.
* Steps in Model
     ...
     + For each stock
          + Fit ARIMA
          + Forecast 6 months
          + Store one expected return
     + Compute covariance matrix (historical)
     + Run hill climbing
          + Perturb weights
          + Recompute Sharpe using fixed expected returns
...
* In this project, ARIMA (AutoRegressive Integrated Moving Average) is used to generate forward-looking estimates of each stock’s daily return before running the hill climbing optimization. Instead of relying solely on historical averages, ARIMA fits a statistical time-series model to each stock’s past returns and forecasts the next expected return using its autoregressive (AR), differencing (I), and moving-average (MA) components.      
* The model learns patterns such as short-term momentum, mean reversion, and noise structure in the return series and produces a predicted next-day value. These ARIMA-based forecasts are then collected into the mean_returns vector and passed into the Sharpe-ratio evaluation function, allowing the hill climbing algorithm to search for an optimal portfolio using predicted returns rather than purely historical ones. This makes the example more realistic by combining a forecasting model with a heuristic search method for portfolio construction. 


## Imports
* Imports & Setup
     + Uses yfinance to retrieve historical stock data.
     + Loads core data science libraries (numpy, pandas, matplotlib).
     + Imports ARIMA from statsmodels for time-series forecasting.
     + Suppresses ARIMA warnings for cleaner output.
...
import matplotlib.pyplot as plt
import time
##using ARIMA to predict forward
from statsmodels.tsa.arima.model import ARIMA
# to ignore ARIMA warnings for cleaner output
import warnings
warnings.filterwarnings("ignore")  

```

## Stock Selection and Fetch
* Stock Selection & Date Range
     + Predefines a list of popular tech and market-moving stocks.
     + Uses a 2-year historical window (Dec 2023 to Dec 2025).
     + Dates can be adjusted depending on analysis needs.
* Data Retrieval
     + Defines a helper function fetch_data() to download adjusted price data.
     + Uses yf.download() with auto-adjusted prices for accuracy.
     + Returns a full historical dataset for all selected tickers
...
## Defining Sharpe Ratio, Risk, and Returns
...
* ARIMA components (p=1, d=1, q=1)
     + If all ARIMA parameters are 1:
     + The series is differenced once to remove trend
     + The model captures momentum with AR(1)
     + The model captures noise with MA(1)
     + Forecasts behave like a smoothed random walk
     + Parameter estimation is relatively simple
     + It’s often a strong baseline for financial forecasting
* We selected 6 months, but can select other time horizons. 
* Can select other component numbers for different effects


```{python}
# --- ARIMA helper: forecast average daily return over next ~6 months (≈126 trading days) ---
def arima_next_6m(series, order=(1, 1, 1), horizon=126):
    series = np.asarray(series)
    model = ARIMA(series, order=order)
    model_fit = model.fit()
    # array of predicted daily returns
    fc = model_fit.forecast(steps=horizon)  
    return float(np.mean(fc)) # average predicted daily return


```

* A moving average (MA) is defined as a smoothing technique where you take the average of the last k number of data points and slide that window forward through time. In contrast, using ARIMA, the moving-average term (MA) does not average recent values. Instead, it models how past forecast errors influence the current value. 
* For the 1st forecast, ARIMA uses:
     + The AR term (yesterday’s differenced return)
     + The MA term (yesterday’s forecast error)
     + The noise estimate σ
* When forecasting into the future past forecast 1, we do not know future errors, because future observations do not exist.
     + For forecasts beyond one step, errors are unknown and assumed 0. The ARIMA model replaces unknown future errors with their expected value = 0.
     + MA(1) uses yesterday’s model error to improve the first forecast. The MA influence fades out after 1 step.
     + Thus, the MA effect vanishes quickly, typically after the first day.

* Over a 126-day horizon, your forecast sequence is dominated by:
     + The AR term
     + The integrated (d=1) structure
     + The model’s drift
     + The variance of historical returns
* The model becomes smoother over long horizons
     + Forecasts converge toward a drift or mean
     + Uncertainty (confidence interval width) grows with each step
     + ARIMA long-term forecasts tend to look like a slowly drifting line, not a reactive curve.


* Is this behavior appropriate for financial returns? Yes, and in fact it's consistent with common financial assumptions:
     + Returns follow a noisy process
     + ARIMA(1,1,1) captures short-term noise but smooths long-term projections.
     + Market returns have low predictability beyond days/weeks
     + Because long-term return forecasts are inherently uncertain, a smoothing ARIMA forecast is often reasonable.
     + You are averaging 126 forecasts
     + So, you're not relying on a precise day-by-day prediction.
     + Instead, you're estimating a long-term expected average daily return, which is a more stable quantity.
     + This is actually a better-behaved target for ARIMA.

* In this code, ARIMA is used before hill climbing, to generate expected returns, and hill climbing then optimizes the weights only.

## Main Loop

* The algorithm records the Sharpe ratio at every successful improvement and stores it in score_history. This allows us to visualize how the portfolio’s performance evolves across iterations.
* The performance function evaluates each portfolio configuration by calculating its Sharpe ratio using:
     + The current portfolio weights
     + The vector of daily mean returns
     + The covariance matrix of asset returns
* This provides a single quantitative score for comparing solutions.
Portfolio weights are always normalized so they sum to 1, ensuring the portfolio remains fully invested.
* Normalization occurs both when:
     + generating the initial random portfolio, and
     + creating each perturbed “neighbor” portfolio during the hill climbing iterations.
...
* Fetch historical prices, compute daily percentage returns, and remove missing values.
* Loop through each stock to estimate expected return.
     + For every ticker, extract its cleaned return series.
* Use ARIMA forecasting when sufficient data exists for longer series, estimate the next 6-month expected return using arima_next_6m().
     + We selected major, highly liquid stocks, each stock would have far more than 30 trading days of return data
     + However, if a stock has fewer than 30 observations, use its historical average return. This was never triggered.
* Convert expected returns to a NumPy array
* Prepares the vector of mean returns for optimization algorithms.
* Compute the covariance matrix of asset returns required for portfolio risk and Sharpe ratio calculations.

```{python}
stock_data = fetch_data(stocks, start_date, end_date)
returns = stock_data['Close'].pct_change().dropna()

## Linking back to ARIMA
mean_returns = []
for s in stocks:
    series = returns[s].dropna()
    if len(series) < 30:
        mean_returns.append(series.mean())
    else:
        mean_returns.append(arima_next_6m(series))  
...
* Interpreting the Sharpe Ratio
     + A Sharpe ratio above 1 is usually considered acceptable
     + Above 2 is very good
     + Above 3 is excellent, indicating that your portfolio offers a high return per unit of risk.

-   Our portfolio is highly concentrated in just a few assets, meaning that the weights are heavily concentrated in a few stocks. The algorithm has effectively zeroed out the remaining assets, which can either be a good or bad thing depending on the actual data.
...
-   A Sharpe ratio of \> 3 is excellent, suggesting that the portfolio has a favorable balance of return vs. risk. This ratio indicates that for each unit of risk, your portfolio earns more than two times the return.
-   Whether this Sharpe ratio is truly impressive depends on the quality and volatility of the returns in the dataset. A very high Sharpe ratio from a simple HC optimizer is unusual in real markets. Financial markets rarely produce consistently high Sharpe ratios, so the high value might indicate the presence of some anomaly, or it could be due to a limited timeframe or low volatility in the assets during this period. It may also indicate model overfitting, or a feature of the simulated behavior rather than tradable results
