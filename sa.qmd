...
```{python}
#Optimize the portfolio based on the ARIMA-predicted average daily return over the next 6 months.

##Using ARIMA model to forecast returns in Simulated Annealing for Portfolio Optimization
## Selected 6 months horizon for return prediction
## Lots of options for tuning ARIMA parameters
...
stocks = ['AAPL', 'GOOGL', 'MSFT', 'AMZN', 'TSLA', 'NFLX', 'NVDA', 'META', 'DIS', 'BA', 'QQQ', 'VTI']
...
def arima_next_6m(series, order=(1, 1, 1), horizon=126):
    ...
    model_fit = model.fit()
    fc = model_fit.forecast(steps=horizon)  # array of predicted daily returns
...
def portfolio_performance(weights, mean_returns, cov_matrix):
     ...
     ##taking care of division by zero issue, which is rare but possible
     if risk == 0:
          sharpe_ratio = -np.inf
     else:
...
# Build ARIMA-based mean_returns: predicted avg daily return over next 6 months
mean_returns_list = []
for s in stocks:
    series = returns[s].dropna()
    if len(series) < 30:  # Fallback if not enough data for ARIMA
        mean_returns_list.append(series.mean())
    else:
        mean_returns_list.append(arima_next_6m(series))
...
start_time = time.time()
best_weights, best_sharpe, sharpe_history = simulated_annealing(
    stocks, mean_returns, cov_matrix, max_evals, initial_temp, cooling_rate
)
end_time = time.time()
execution_time = end_time - start_time
...
plt.title("Simulated Annealing Progress for Portfolio Optimization (Sharpe Ratio, 6-Month ARIMA)")
plt.xlabel("Iteration")
